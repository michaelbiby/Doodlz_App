...
public class DoodleView extends View {
	...
	private void touchMoved(MotionEvent event) {
		...
		for (int i = 0; i < event.getPointerCount(); i++) {
			...
			if (pathMap.containsKey(pointerID)) {
				...
				float newX_coordinate = event.getX(pointerIndex);
				float newY_coordinate = event.getY(pointerIndex);
				...
				float deltaX = Math.abs(newX_coordinate - point.x);
				float deltaY = Math.abs(newY_coordinate - point.y);

				// if the distance is significant enough to matter
				if (deltaX >= TOUCH_TOLERANCE || deltaY >= TOUCH_TOLERANCE) {
					// move the path to the new location
					path.quadTo(point.x, point.y,
							(newX_coordinate + point.x) / 2,
							(newY_coordinate + point.y) / 2);

					// store the new coordinates
					point.x = (int) newX_coordinate;
					point.y = (int) newY_coordinate;
				}
			}
		}
	...
	public void saveImage() {
		...
		String location = MediaStore.Images.Media.insertImage(getContext()
				.getContentResolver(), bitmap, name, "Doodlz Drawing");
